...
Sort the inventors by birth date in ascending order 
(from those born furthest in
...
firstLast = people.map((person) => {
  const [last, first] = person.split(", "); // Split into last and first name
  return `${first} ${last}`; // Rearrange to "First Last"
});
// Check your work:
...
isAdultPresent = devs.some((dev) => dev.year < date - 18);
// Check your work:
...
// Complete the exercise in the space below:
isEveryone19OrOlder = devs.every((dev) => dev.year < date - 19);
// Check your work:
...
// Complete the exercise in the space below:
totalYearsLived = inventors.reduce((count, inventor) => {
...
// Complete the exercise in the space below:
travelMethodCounts = travelMethods.reduce((count, travelMethod) => {
  if (!count[travelMethod]) {
    count[travelMethod] = 1;
  } else {
    count[travelMethod]++;
  }
  return count;
